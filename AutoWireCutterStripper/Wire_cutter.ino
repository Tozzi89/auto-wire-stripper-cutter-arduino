...
Servo linMotSteppers;
int linMotSteppersCurrStep = 0;
...
const int LINMOT_STEPPERS_STEPS = 1;  // Steppers step(s) movement at a time. (Ersätt för servo/grader) Klar?
const int EXTRUDER_STEPPER_STEPS = 1;

const int EXTRUDER_STEPPER_SPEED = 2000;


const int SCREEN_WIDTH = 128;  // OLED display width, in pixels 128
const int SCREEN_HEIGHT = 64;  // OLED display height, in pixels 64
...
const int CUTTING_STEPS = 25; // Degrees for cutting wire
const int STRIPPING_MULTIPLIER = 300;  // The chosen stripping depth value on the screen is multiplied by this value. // Ändra/kalibrera för servo
...
void setup() {
    linMotSteppers.attach(9);
//    linMotSteppers.write(0); // Onödig?
...
void drawWire() {
    display.drawLine(0, WIRE_STRAND_Y_POS, WIRE_STRAND_LENGTH, WIRE_STRAND_Y_POS, SSD1306_WHITE);
    display.fillRect(WIRE_STRAND_LENGTH, 0, WIRE_INSULATION_WIDTH, WIRE_INSULATION_HEIGHT, SSD1306_WHITE);
    display.drawLine(WIRE_STRAND_LENGTH + WIRE_INSULATION_WIDTH, WIRE_STRAND_Y_POS, SCREEN_WIDTH, WIRE_STRAND_Y_POS, SSD1306_WHITE);
}
...
void drawComponent(Component comp) {
    if (comp.highlighted) {
        ...
        display.fillRect(comp.x, comp.y, comp.w, comp.h, SSD1306_WHITE);

        if (comp.selected) {
            display.drawRect(comp.x - 1, comp.y - 1, comp.w + 2, comp.h + 2, SSD1306_WHITE);
        }

    }
    else {
        display.setTextColor(SSD1306_WHITE, SSD1306_BLACK);
        display.drawRect(comp.x, comp.y, comp.w, comp.h, SSD1306_WHITE);
    }
...
void moveBlade(int steps) {
    linMotSteppers.write(steps);  // Ändra för servo! Klar?
    delay(10);
    linMotSteppers.write(-(steps));
    delay(10);
//    linMotSteppersCurrStep += steps; // Radera?
}
